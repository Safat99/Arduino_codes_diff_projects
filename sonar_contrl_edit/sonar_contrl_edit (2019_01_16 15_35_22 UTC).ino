...
void loop() {
  ...
  while(1){
    ...
    motor_control(0, 0);
    digitalWrite(lft_led,HIGH);digitalWrite(rht_led,HIGH);//digitalWrite(lft_led,HIGH);digitalWrite(lft_led,HIGH);
    delay( 100);
    sonar_activate();
  while(1){
    sonar_activate();
    motor_control(100, -100);
    if((dist[1]>=3 && dist[1]<=45) && (dist[0]>=15 && dist[0]<=45) ){
      digitalWrite(lft_led,0);digitalWrite(rht_led,0);
      break;
    }
  }
//  else{
