...
//  delay(1000);
//  callibration();
//  while(trial==0 && solve==0){
//    trial=digitalRead(trial_s);
//    solve=digitalRead(solve_s);
//  }
//  if(trial==1){
//    for(int i=0; i<5; i++){
...
//    solve_maze();
//    int i=1;
//    while(solve_array[i]!=0){
//      Serial.print(solve_array[i]);
//      i++;
//    }
//    for(int i=0; i<5; i++){
//      digitalWrite(rht_led, 1);
//      delay(200);
//      digitalWrite(rht_led, 0);
//      delay(200);
//    }
//  }
////  pos-=EEPROM.read(0);
...
//LED Pin
  ...
  //Switch
  pinMode(trial_s,INPUT);
  pinMode(solve_s,INPUT);
}
void d_sensor_read(){
  sum=0;
  for(int i=0; i<10; i++){
    d_sensor[i]= analogRead(sensor_pin[i]) >2000;
    if(d_sensor[i]){
      sum++;
    }
...
void PD(){
 ...
 int left_speed, right_speed;
  int prev_error;
  left_speed = 120 + (Kp * error +Kd*(error-prev_error)) ;
  right_speed = 120 - (Kp * error +Kd*(error-prev_error)) ;
  prev_error=error;
//  if (error<-1){
//    right_speed+=60;
//    left_speed-=60;
...
void switchs(){
  ...
  else  digitalWrite(lft_led,0);
  if (digitalRead(PB14)){
    digitalWrite(rht_led,1);
    }
  else  digitalWrite(rht_led,0);
  }
...
void solve_maze(){
  ...
  for(int i=1; i<=pos; ){
    ...
    if(c=='P'){
      ch_array[i]= EEPROM.read(i);
      ch_array[i+1]= EEPROM.read(i+1);
      ch_array[i+2]= EEPROM.read(i+2);
      if(ch_array[i+1]=='U'){
        if(ch_array[i+2]=='U'){
          solve_array[i]='L';
          i+=3;
